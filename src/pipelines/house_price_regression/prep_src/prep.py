...
price_data = df_list[0]
# green_data = df_list[0]
# yellow_data = df_list[1]

# Define useful columns needed for house price ml
...
def cleanDataTest(data):
    new_df = (data.dropna(how="all"))[useful_columns]
    print("Columns after dropna")
    print(new_df.columns)
    return new_df

output_data = cleanDataTest(price_data)
output_price = output_data.to_csv((Path(args.prep_data) / "house_price_prep_data.csv"))
